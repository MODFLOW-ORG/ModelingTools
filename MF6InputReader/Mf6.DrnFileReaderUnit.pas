...
type
  ...
  private
    ...
    procedure Read(Stream: TStreamReader; Unhandled: TStreamWriter;
      Dimensions: TDimensions; naux: Integer; BOUNDNAMES: Boolean; Options: TDrnOptions;
        PriorPeriod: TDrnPeriod);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if not Assigned(ELEV) then
        begin
          Unhandled.WriteLine(Format('Failed to read ELEV in %s', [FPackageType]));
          Exit;
        end;
        ...
        begin
          ...
          begin
            for var ColIndex := 0 to FDimensions.NCol - 1 do
            begin
              ELEVValue := ELEV[LayerIndex, RowIndex, ColIndex];
              ...
              if Abs((InactiveValue - ELEVValue)/InactiveValue) < 1e-10 then
              begin
                Continue;
              end;
              ...
              Cell.Felev.NumericValue := ELEVValue;
    ...
    if Options.READARRAYGRID then
    begin
      SetLength(ELEV, FDimensions.NLay, FDimensions.NRow, FDimensions.NCol);
